"""Tests for HCS (High Content Screening) support with tensorstore backend."""
...
pytest.importorskip("tensorstore", reason="tensorstore backend not available")
pytest.importorskip("useq", reason="useq not available")
pytest.importorskip("yaozarrs", reason="yaozarrs not available")


def test_hcs_plate_with_tensorstore(tmp_path: Path) -> None:
    """Test creating HCS plate structure with tensorstore backend."""
...
def test_hcs_plate_with_multiple_fields_per_well_tensorstore(tmp_path: Path) -> None:
    """Test HCS plate with multiple fields of view per well using tensorstore."""
...
def test_hcs_plate_from_useq_tensorstore(tmp_path: Path) -> None:
    """Test HCS plate from useq.MDASequence with tensorstore."""
    ...
    assert output_path.exists()
    plate_path = output_path / (plate.name or "plate")
    assert plate_path.exists()
...
def test_hcs_large_plate_from_useq_tensorstore(tmp_path: Path) -> None:
    """Test HCS large plate with multi-letter rows and tensorstore."""
...
def test_hcs_metadata_validation_tensorstore(tmp_path: Path) -> None:
    """Test HCS metadata validation with yaozarrs and tensorstore."""
    from useq import GridRowsColumns, MDASequence, WellPlatePlan
    ...
    plate_name_sanitized = (plate.name or "plate").replace(" ", "_")
