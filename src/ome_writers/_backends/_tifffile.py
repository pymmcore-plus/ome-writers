...
class TiffBackend(ArrayBackend):
    ...
    def _update_bounded_metadata(self) -> None:
        """Update OME metadata for bounded dimensions with frame metadata.

        For bounded dimensions, the base OME metadata was already written during
        prepare(). This method enhances it with Plane objects and StructuredAnnotations
        from frame_metadata and writes the updated metadata to the TIFF files.
        """
        ...
        for p_idx, writer in sorted(self._position_writers.items()):
            try:
                if len(self._position_writers) > 1:  # multiposition: full OME
                    ...
                    # Use enhanced image from cached metadata
                    xml = ome.OME(
                        images=[self._cached_metadata.images[p_idx]],
                        structured_annotations=self._cached_metadata.structured_annotations,
                    ).to_xml()
